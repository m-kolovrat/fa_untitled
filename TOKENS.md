# Design tokens: Figma ↔ code

How a change made to the **Figma design system** ("❖ Untitled UI x Fa") maps to a change in
**this repo**. The goal: a designer says _"I changed X"_ and an engineer can find exactly one
place named `X`.

---

## 1. The principle — names match on purpose

Untitled UI keeps **1:1 naming between Figma variables and code tokens** (for colors). The
_leaf_ name is identical at every layer; only Figma's **group path** becomes the code **prefix**.

| Layer                          | Example                                                                   |
| ------------------------------ | ------------------------------------------------------------------------- |
| Figma variable                 | `Colors/Background/` **`bg-brand-solid`**                                 |
| CSS token (`styles/theme.css`) | `--color-` **`bg-brand-solid`**                                           |
| Tailwind utility               | **`bg-brand-solid`**                                                      |
| Component usage (`button.tsx`) | `className="… `**`bg-brand-solid`**` hover:`**`bg-brand-solid_hover`**`"` |

Modifiers carry through too: `_hover`, `_alt`, `_on-brand`, `_subtle`.

### Figma group → code prefix

| Figma path                 | Code prefix        | Example                  |
| -------------------------- | ------------------ | ------------------------ |
| `Colors/Background/…`      | `--color-bg-…`     | `bg-brand-solid`         |
| `Colors/Text/…`            | `--color-text-…`   | `text-primary`           |
| `Colors/Foreground/…`      | `--color-fg-…`     | `fg-brand-secondary`     |
| `Colors/Border/…`          | `--color-border-…` | `border-primary`         |
| `Colors/Effects/Shadows/…` | `--shadow-…`       | `shadow-xs-skeuomorphic` |

---

## 2. Where tokens live

| Kind                     | File                                                                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Primitives (ramps)       | `styles/theme.css` `@theme {}`                                         | All 12 Figma ramps are canonical, synced FA ramps — `brand`, `error`, `gray` (split light/dark, `@theme` + `.dark-mode`), `warning`, `success`, `blue-light`, `purple`, `pink`, `orange-dark`, `teal` (retained, unreferenced), plus `white`/`black`/`transparent`. The Tailwind stock palette is reset (`--color-*: initial`) so nothing silently falls back to a Tailwind default — an untokenized color renders as nothing, on purpose. |
| Semantic aliases         | `styles/theme.css` `@theme {}`                                         | e.g. `--color-bg-brand-solid: var(--color-brand-500)`                                                                                                                                                                                                                                                                                                                                                                                      |
| Dark-mode overrides      | `styles/theme.css` `.dark-mode {}`                                     | each mode has its **own** binding — edit the mode you changed in Figma                                                                                                                                                                                                                                                                                                                                                                     |
| Type scale               | `styles/theme.css` (`--text-*`)                                        | `text-xs … text-display-2xl`                                                                                                                                                                                                                                                                                                                                                                                                               |
| Radius scale             | `styles/theme.css` (`--radius-*`)                                      | `xxs 2px … 4xl 24px`; `none`/`full` are Tailwind's hardcoded 0 / infinity, not theme-configurable                                                                                                                                                                                                                                                                                                                                          |
| Component geometry       | the component's `sortCx` styles                                        | radius / padding / size, e.g. `components/base/buttons/button.tsx`                                                                                                                                                                                                                                                                                                                                                                         |
| Motion (duration/easing) | `styles/theme.css` `@theme {}` (`--transition-duration-*`, `--ease-*`) | **Not** synced from Figma — see §8.                                                                                                                                                                                                                                                                                                                                                                                                        |

---

## 3. The three layers — decide where a change goes

When a Figma change lands, classify it, then edit the matching layer:

| #                  | Change is…                                                                | Goes in                                          | Scope                            |
| ------------------ | ------------------------------------------------------------------------- | ------------------------------------------------ | -------------------------------- |
| **Primitive**      | a ramp step's hex value (e.g. `brand-600 = #035CB5`)                      | `--color-brand-N` in `theme.css`                 | cascades to **everything** brand |
| **Semantic alias** | what a role points to (e.g. `bg-brand-solid → brand-500`)                 | `--color-<name>` in `theme.css`                  | cascades to **that role**        |
| **Component**      | geometry or a component-specific class (radius, padding, a one-off color) | the component's `styles` map (e.g. `button.tsx`) | **that component only**          |

Rule of thumb: change the **token** when the whole system should follow; change the
**component** when only that component should.

---

## 4. Colors vs. dimensions caveat

The perfect 1:1 naming holds for **colors** (primitives _and_ semantics). For **radius / spacing**,
the _value_ matches Figma but the _name_ follows **Tailwind's** utility convention:

| Figma variable       | Code                | Match                         |
| -------------------- | ------------------- | ----------------------------- |
| `bg-brand-solid`     | `bg-brand-solid`    | name identical ✅             |
| `brand-600`          | `--color-brand-600` | name identical ✅             |
| `radius-full` (9999) | `rounded-full`      | value matches, Tailwind name  |
| `spacing-md` (8)     | `p-2` / `gap-2`     | value matches, Tailwind scale |

So colors you can nearly **grep by their Figma name**; dimensions you **translate to Tailwind**.

### Two intentional divergences

- **Alpha tokens.** Figma enumerates 20 discrete steps (`alpha-white-10` …
  `alpha-black-100`). Code uses one base colour plus Tailwind's opacity modifier:
  `bg-alpha-white/20` is exactly Figma's `alpha-white-20`, including the dark-mode
  swap to `rgb(12 14 18)`. Equivalent and more flexible — not ported 1:1.
- **Spacing & containers.** Figma's `spacing-*` alias onto a 4px base identical to
  Tailwind's numeric scale (names not emitted — `p-2` stays `p-2`, not `p-md`).
  `container-max-width-desktop` (1280) already exists as `--max-width-container`;
  container padding is the `px-4 md:px-8` convention. Neither is emitted as its own
  token — the values already match, so there's nothing to sync.

---

## 5. Worked examples (from this pilot)

Three real edits, one per layer:

**Radius → pill** _(component layer)_
Figma: rebound the button's corner radius `radius-sm → radius-full`.
Code: `components/base/buttons/button.tsx`, `sizes` map — `rounded-lg → rounded-full`
(and the inner border `before:rounded-[7px] → before:rounded-full`), across all sizes.

**Brand → FA blue** _(primitive layer)_
Figma: brand ramp `25 … 950` hex values.
Code: `--color-brand-25 … 950` in `styles/theme.css`. One edit reskinned every brand token
(button fill, focus ring, brand text/borders, subtle brand backgrounds).

**BG Brand Solid → brand-500** _(semantic layer)_
Figma: alias `bg-brand-solid` rebound `brand-600 → brand-500` (hover `brand-700 → brand-600`).
Code: `--color-bg-brand-solid: var(--color-brand-500)` and `…_hover: var(--color-brand-600)`
in `styles/theme.css`. Light mode only — dark mode keeps its own binding.

**Error → FA red ramp** _(primitive layer)_
Figma: the `error` ramp `25 … 950` hex values (Figma has no "red"/"utility-red" — only `error`).
Code: added `--color-error-25 … 950` in `styles/theme.css`, then repointed every error consumer
off Tailwind's default `--color-red-*` and onto the new ramp — both the error semantics
(`text-error-primary`, `bg-error-solid`, `border-error`, `fg-error-*`, `focus-ring-error`) **and**
the `--color-utility-red-*` tier used by error Badges and FeaturedIcons. The `utility-red` tokens
keep their code name but now resolve to `--color-error-*`. Light + dark both rebound (dark keeps
its inverted step mapping, e.g. `utility-red-50 → error-950`).

---

## 6. Reading Figma values (tooling notes)

- The Figma MCP `get_variable_defs` resolves **values**, but only for variables a **selected
  node/layer actually uses** — point it at the button, you get the ~4 shades the button uses.
- `search_design_system` lists variable **names** across libraries, but **not their values**.
- **Whole collections are readable directly** via the Figma plugin API —
  `getLocalVariableCollectionsAsync()` + `variable.valuesByMode`, including per-mode values
  and alias chains (a variable pointing at another variable, not a literal). This is how
  `tokens/figma-tokens.json` — the repo's committed Figma extract — gets built: every
  primitive step, every semantic `[light, dark]` pair, and every alias in one pass, no
  per-node selection needed. There's no longer a reason to ask the designer to paste hexes
  for a full ramp.

---

## 7. Verify a change

```bash
npm install --legacy-peer-deps   # first time only (eslint peer conflict)
npm run tokens:check             # theme.css vs. tokens/figma-tokens.json parity
npm run storybook                # → http://localhost:6006
```

- **`npm run tokens:check`** (`scripts/check-token-parity.mjs`) diffs `theme.css` against
  `tokens/figma-tokens.json` and fails on anything missing, bound to the wrong ramp step, or
  present in code but absent from Figma. Runs in CI on every push/PR — the next Figma drift
  should show up here, not as a visual regression days later.
- **Foundations → Colors / Typography / Effects** — auto-generated from `theme.css` (Vite `?raw`),
  so it reflects token edits immediately.
- **Base components / Buttons / Primary** — the pilot button (sizes + states).

---

## 8. Motion — not yet in Figma

Everything above (§1–7) covers colour, radius, spacing, and type — the properties the committed
`tokens/figma-tokens.json` extract actually contains. **Motion (`--transition-duration-*`,
`--ease-*` in `styles/theme.css`) has no Figma collection today**, so it sits outside that
extract and outside `npm run tokens:check` — the parity script will not flag a motion edit,
correct or not, because it only diffs colour/radius/spacing/type against Figma.

Until a motion collection exists in the Figma library, **code is the source of truth** for these
values: they were derived from the `emil-design-eng` design-engineering skill grounded in what
was already in the codebase, not synced from a design file. If Figma gains a motion spec later,
reconcile it here the same way §5's worked examples do, and consider extending
`check-token-parity.mjs` to cover it.

See **Foundations → Motion** in Storybook for the full scale, and the "CSS Transitions" section
of `CLAUDE.md` for the house convention (hover is instant; the scale is for movement and
overlays only).
